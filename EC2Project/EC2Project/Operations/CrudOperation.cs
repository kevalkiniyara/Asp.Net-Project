...
namespace EC2Project.Operations
{
    public class CrudOperation : ICrudOperation
    {
        ...
        public CaseModel UpdateModel(CaseModel model)
        {
            var modelDetail = _context.CaseModels.Where(x => x.ID == model.ID).FirstOrDefault();
            if(modelDetail!=null)
            {
                ...
                //modelDetail.RegisterDate =model.RegisterDate;
            }
            _context.SaveChanges();
            return modelDetail;
        }
